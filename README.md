...
22. key-traffic-light: the Key cell in any library table is highlighted green when Camelot-compatible with the currently playing "master" deck's key (most recently started of the playing decks; nothing highlighted while nothing is playing). Side effect: the Key column everywhere now always shows musical note names, never Camelot numbers, regardless of the Preferences key-notation setting.

Checked here: all twenty-two apply cleanly, in this exact order, on the unpatched Debian source (fresh extraction, tested end to end). Not checked here: compiling (that is what the GitHub build does) or hearing/using any of it (that is what the Pi does).
